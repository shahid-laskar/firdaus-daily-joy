        ...
        {tabs.map((t) => {
          ...
          return (
            <button
              ...
              className={`press relative shrink-0 rounded-full px-3.5 py-1.5 text-[0.8rem] font-medium whitespace-nowrap transition-colors ${
                active
            ...
            >
              {t.label}
              {active && (
                <span className="bg-space absolute inset-x-3.5 -bottom-px h-[2px] rounded-full" />
              )}
            </button>
          );
        })}
...
export function EmptyState({
  ...
  return (
    <div className="border-border/70 rounded-2xl border border-dashed px-6 py-10 text-center">
      <div className="text-ink-faint/70 font-display mx-auto mb-3 text-3xl leading-none">
        {glyph}
      </div>
